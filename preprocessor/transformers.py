...
def tracked_first_value(
    ad: dict,
    params: dict,
    names: list[str],
    consumed_param_keys: set[str],
    prefer_params: bool = True,
):
    for name in names:
        if prefer_params and params.get(name) not in (None, "", [], {}):
            consumed_param_keys.add(name)
...
def transform_records(raw_records: list[dict]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    for raw_record in raw_records:
        ...
        consumed_param_keys = set(LISTING_PARAM_KEYS).intersection(params)
        ...
        if not append_detail_row(detail_rows, group, list_id, ad, params, title, price, consumed_param_keys):
            skipped_list_ids.add(list_id)
            continue

        for attr_id, attr_value in params.items():
            if attr_id in consumed_param_keys:
                continue
            attributes.append(
                {
                    "list_id": list_id,
...
def append_detail_row(
    ...
    title: str | None,
    price: float | None,
    consumed_param_keys: set[str] | None = None,
) -> bool:
    consumed_param_keys = consumed_param_keys if consumed_param_keys is not None else set()

    def first_value(ad: dict, params: dict, names: list[str], prefer_params: bool = True):
        return tracked_first_value(ad, params, names, consumed_param_keys, prefer_params)

    if group == "real_estate":
        area_m2 = to_number(first_value(ad, params, ["size", "living_size"]))
        consumed_param_keys.update({"type", "street_name"}.intersection(params))
        ...
        consumed_param_keys.update({"job_urgent_recruit_enabled"}.intersection(params))
        ...
        min_price = to_number(first_value(ad, params, ["min_price", "price"]))
        ...
        item_detail = first_body_segment(ad.get("body"))
        if item_detail in (None, "", [], {}):
            return
